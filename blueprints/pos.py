...
def create_order():
    ...
    try:
        ...
        # 2. Walk-in Patient Creation
        patient_uid = f"CD-PAT-{str(uuid.uuid4())[:6].upper()}"
        cursor.execute("""
            INSERT INTO patients (patient_uid, full_name, status) 
            VALUES (%s, 'Walk-in Patient', 'ACTIVE') RETURNING id
        """, (patient_uid,))
        ...
        cursor.execute("""
            INSERT INTO orders (order_ref, patient_id, source) 
            VALUES (%s, %s, %s) RETURNING id
        """, (order_ref, patient_id, idempotency_key))
        ...
        for test_id in data.get('tests', []):
            ...
            subtotal += price
            cursor.execute("""
                INSERT INTO test_components (order_id, test_id, status) 
                VALUES (%s, %s, 'LOGGED')
            """, (order_id, test_id))
        ...
        cursor.execute("""
            INSERT INTO invoices (order_id, invoice_ref, subtotal, discount, total_amount, status, issued_at) 
            VALUES (%s, %s, %s, %s, %s, 'ISSUED', CURRENT_TIMESTAMP) RETURNING id
        """, (order_id, inv_ref, subtotal, discount, total))
        ...
        if advance > 0:
            ...
            """, (invoice_id, advance, data.get('method', 'Cash'), str(uuid.uuid4()), session.get('role')))
            cursor.execute("UPDATE invoices SET status = %s WHERE id = %s", (status, invoice_id))
            
        conn.commit()
        return jsonify({"status": "success", "invoice_ref": inv_ref})
